...
export class LayoutService {
  ...
  // Update base config on current runtime, not update for setting localStorage
  tempUpdateBaseConfig(config: ILayout) {
    this.layoutConfigSubject.next(config);
  }

  updateContainer(containerType: Exclude<IContent['container'], undefined>) {
    const currentConfig = { ...this.layoutConfigSubject.value };
    if (
      currentConfig &&
      currentConfig.app &&
      currentConfig.app.toolbar &&
      currentConfig.app.content
    ) {
      currentConfig.app.toolbar.container = containerType;
      currentConfig.app.content.container = containerType;
      this.tempUpdateBaseConfig(currentConfig);
    }
  }
}
