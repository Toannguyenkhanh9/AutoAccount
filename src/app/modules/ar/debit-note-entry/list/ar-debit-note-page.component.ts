...
export class ArDebitNotePageComponent {
  ...
  items = [
    { code:'ADJ-FEE', name:'Administrative Fee', price: 50 },
    { code:'DAMAGED', name:'Damaged packaging charge', price: 35 },
    { code:'RET-RESTOCK', name:'Restocking Fee', price: 80 },
  ];

  // ===== Data =====
  notes: DebitNote[] = [
    {
      dnNo:'DN-0001', dnDate:'2025-08-10',
      debtor:'300-C001', debtorName:'CARE PHONE SDN BHD',
      currency:'MYR', reference:'RET-15',
      status:'POSTED', narration:'Restocking fee',
      subTotal:80, taxTotal:4.8, rounding:0, grandTotal:84.8,
      lines:[
        { item:'RET-RESTOCK', description:'Restocking Fee', qty:1, price:80, taxCode:'SR', taxRate:6, taxAmt:4.8, lineTotal:80 }
      ]
    },
    {
      dnNo:'DN-0002', dnDate:'2025-08-12',
      debtor:'300-B001', debtorName:'BEST PHONE SDN BHD',
      currency:'MYR', reference:'ADJ-02',
      status:'POSTED', narration:'Admin fee',
      subTotal:50, taxTotal:0, rounding:0, grandTotal:50,
      lines:[
        { item:'ADJ-FEE', description:'Administrative Fee', qty:1, price:50, taxCode:'EX', taxRate:0, taxAmt:0, lineTotal:50 }
      ]
    }
  ];

  // ===== List / filter / paging =====
  q=''; sortBy: keyof DebitNote='dnDate'; sortDir:'asc'|'desc'='desc';
  page=1; pageSize=8;
  selected?: DebitNote;
  ...
  setSort(k:keyof DebitNote){ if(this.sortBy===k) this.sortDir=this.sortDir==='asc'?'desc':'asc'; else{ this.sortBy=k; this.sortDir='asc'; } }
  selectRow(n:DebitNote){ this.selected=n; }
  ...
  refresh(){ this.q=''; this.page=1; this.sortBy='dnDate'; this.sortDir='desc'; }

  // ===== Dialog / forms =====
  showForm=false; formMode:'new'|'edit'|'view'='new';
  dnForm!: FormGroup;
  get linesFA(): FormArray<FormGroup>{ return this.dnForm.get('lines') as FormArray<FormGroup>; }

  showFind=false; findForm!: FormGroup; findResults: DebitNote[]=[];
  ...
  private nextNumber(){ const seq=(this.notes.length+1).toString().padStart(4,'0'); return `DN-${seq}`; }

  buildForms(){
    this.dnForm=this.fb.group({
      dnNo: ['', Validators.required],
      dnDate: [this.today(), Validators.required],
    ...
    this.printForm=this.fb.group({
      from:[''], to:[''], debtor:[''], sort:['dnDate' as keyof DebitNote]
    });
  }
  ...
  deleteNote(){
    ...
    if(confirm(`Delete ${this.selected.dnNo}?`)){
      this.notes=this.notes.filter(x=>x!==this.selected); this.selected=undefined;
    }
  }

  private patch(n:DebitNote){
    this.dnForm.patchValue({
      dnNo:n.dnNo, dnDate:n.dnDate, debtor:n.debtor, debtorName:n.debtorName,
  ...
  saveNote(){
    if(this.dnForm.invalid){ this.dnForm.markAllAsTouched(); return; }
    ...
    if(this.formMode==='new'){
      ...
      const i=this.notes.findIndex(x=>x.dnNo===this.selected!.dnNo);
  ...
  // Header changes
  onDebtorChanged(){
    const code=this.dnForm.get('debtor')!.value as string;
    const name=this.debtors.find(d=>d.code===code)?.name || '';
    this.dnForm.patchValue({debtorName:name},{emitEvent:false});
  }

  // Lines
  buildLine(l?:Partial<DNLine>){
  ...
  recalcTotals(){
    ...
    const rounding = +this.dnForm.get('rounding')!.value || 0;
    const grandTotal = +(subTotal + taxTotal + rounding).toFixed(2);
    this.dnForm.patchValue({ subTotal, taxTotal, grandTotal }, {emitEvent:false});
  }

  private valueFromForm(): DebitNote{
    const fv=this.dnForm.getRawValue();
    const lines:DNLine[]=this.linesFA.controls.map(f=>f.getRawValue() as DNLine);
    const debtorName=this.debtors.find(d=>d.code===fv.debtor)?.name || '';
    return {
      dnNo:fv.dnNo, dnDate:fv.dnDate, debtor:fv.debtor, debtorName,
  ...
  pickFromFind(n:DebitNote){ this.selected=n; this.showFind=false; }

  openPrint(){ this.showPrint=true; this.showPreview=false; }
buildListing(): DebitNote[] {
  // Ép kiểu giá trị form để có sort?: keyof DebitNote
  const f = this.printForm.value as {
    from?: string; to?: string; debtor?: string; sort?: keyof DebitNote;
  };

  const from = f.from ? new Date(f.from) : undefined;
  const to   = f.to   ? new Date(f.to)   : undefined;

  // Khóa sắp xếp có kiểu rõ ràng
  const sortKey: keyof DebitNote = (f.sort ?? 'dnDate') as keyof DebitNote;

  return [...this.notes] // copy trước khi sort
    .filter(n => {
      const dt = new Date(n.dnDate);
      const inDate = (!from || dt >= from) && (!to || dt <= to);
      const debtorHit = !f.debtor || n.debtor === f.debtor;
