...
@Component({
  selector: 'app-ap-debit-note-page',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule],
  templateUrl: './ap-debit-note-page.component.html',
  styleUrls: ['./ap-debit-note-page.component.scss']
})
export class ApDebitNotePageComponent {
  Math = Math;
  ...
  rows: ApDebitNote[] = [
    {
      ...
      rate: 1,
      status: 'POSTED',
      remark: 'Price adjustment for phones',
      lines: [{ uom: 'UNIT', qty: 1, unitPrice: 500, tax: 0, amount: 500 }],
      localTotal: 500
    },
    {
      docNo: 'APDN-00002',
      ...
      rate: 1,
      status: 'POSTED',
      remark: 'Freight surcharge',
      lines: [{ uom: 'SERVICE', qty: 1, unitPrice: 120, tax: 6, amount: 127.2 }],
      localTotal: 127.2
    }
  ];

  q = '';
  selected?: ApDebitNote;
  page = 1;
  pageSize = 15;
  ...
  private initForm(row?: ApDebitNote) {
  ...
  openEdit(r: ApDebitNote) { this.viewMode = 'edit'; this.selected = r; this.initForm(r); this.show = true; }
  openView(r: ApDebitNote) { this.viewMode = 'view'; this.selected = r; this.initForm(r); this.form.disable(); this.show = true; }
  ...
  private nextDocNo(): string {
    ...
    return `APDN-${(last + 1).toString().padStart(5, '0')}`;
  }
}

/** Optional route config */
export const AP_DEBIT_NOTE_ROUTES: Routes = [
  { path: '', component: ApDebitNotePageComponent }
];
