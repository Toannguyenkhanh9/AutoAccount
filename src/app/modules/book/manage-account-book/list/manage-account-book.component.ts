...
import { ConfigNewDbComponent } from 'src/app/_shared/partials/share-form/config-new-db/config-new-db.component';
...
export class ManageAccountBookComponent implements OnInit, OnDestroy {
  ...
  private manageAccountBookService = inject(ManageAccountBookService);
  ...
  confirmDelete() {
    ...
    this.manageAccountBookService.deleteBook(id).subscribe({
      next: () => {
        // capture một lần
  ...
  saveEdit() {
    ...
    this.manageAccountBookService.editBook(id, updated).subscribe({
      next: (res) => {
    ...
    // cập nhật trên UI
    this.listserver[this.selectedIndex] = updated;
    this.showEdit = false;
  }
  attach() {
    this.openAttach();
